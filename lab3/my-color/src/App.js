...
const palettes = [ // Масив об'єктів з даними палітр
  {
    "paletteName": "Material UI Colors", // Назва палітри
    "id": "material-ui-colors", // Ідентифікатор
    "emoji": "🎨", // Emoji для палітри
    "colors": [ // Масив кольорів з назвами та hex-кодами
...
function App() {
  ...
  const [overlay, setOverlay] = useState({ visible: false, color: '', text: '' });
  ...
  const handleSelectColor = (colorObj) => {
    let displayValue = colorObj.color; // Default to HEX
    if (copyFormat === 'rgb') {
      displayValue = hexToRgb(colorObj.color);
    } else if (copyFormat === 'rgba') {
      let rgb = hexToRgb(colorObj.color);
    ...
    setOverlay({ visible: true, color: colorObj.color, text: `${colorObj.name} (${displayValue})` });
    setTimeout(() => {
  ...
  return (
    <div className="App">
      <header>
        <h1>Flat UI Colors Lab</h1>
      </header>

      {/* Main screen displaying palettes in a 3x3 grid */}
      {!selectedPalette ? (
        <div id="paletteView" className="palette-container">
          {palettes.map(palette => (
            ...
            >
              <h3>{palette.emoji} {palette.paletteName}</h3>
              <div>
                {palette.colors.slice(0, 20).map((c, i) => (
                  <div
                    key={i}
        ...
        <div id="colorView">
          <div className="color-view-header">
            <button onClick={handleBack} className="color-back-button">&larr; Back</button>
            <div className="format-selector">
              Copy Format:
              <select value={copyFormat} onChange={handleFormatChange}>
          ...
          <div id="colorBoxes" className="color-boxes">
          ...
          <div id="paletteFooter" className="palette-footer">
            {selectedPalette.paletteName} by {selectedPalette.emoji || "Unknown"}
          </div>
        </div>
      )}
      ...
      {overlay.visible && (
        <div
          ...
          style={{ backgroundColor: overlay.color, display: 'flex' }}
      ...
      <audio ref={audioRef}>
        ...
        Ваш браузер не підтримує відтворення звуку.
      </audio>
    </div>
  );
}
